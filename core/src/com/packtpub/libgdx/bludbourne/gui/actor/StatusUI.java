...
import com.packtpub.libgdx.bludbourne.gui.actor.inventoryButton.InventoryButtonListener;
...
public class StatusUI extends Window {
    ...
    public StatusUI(){
        ...
        Label level = new Label(String.valueOf(_levelVal), Utility.STATUSUI_SKIN);

        Label goldLabel = new Label(" gp:", Utility.STATUSUI_SKIN);
        Label gold = new Label(String.valueOf(_goldVal), Utility.STATUSUI_SKIN);

        //buttons
        _inventoryButton = new ImageButton(Utility.STATUSUI_SKIN, "inventory-button");
        _inventoryButton.getImageCell().size(32,32);

        _inventoryButton.addListener(new InventoryButtonListener(InventoryButtonObservable.getInstance()));
